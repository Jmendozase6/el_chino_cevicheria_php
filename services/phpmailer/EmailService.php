...
class EmailService
{
    ...
    public function sendContactUsEmail($name, $emailFrom, $subject, $content): bool
    {
        try {
            ...
            $mail->Body =
                ...
                <p> Tu mensaje ha sido enviado con éxito. <br>
                <p> Nos comunicaremos contigo lo más pronto posible. <br>
                <p> Si no has enviado un mensaje, ignora este mensaje.</p >';
            $mail->AltBody = 'Correo electrónico de contacto';
            return $mail->send();
    ...
    public function sendOrderEmail($email, $subject, $orderId, $total): bool
    {
        try {
            $mail = $this->getMailConfigBase();
